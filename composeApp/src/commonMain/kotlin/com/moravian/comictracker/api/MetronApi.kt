...
private val HOME_SAFE_PUBLISHERS =
    setOf(
        "archie comics",
        "boom! studios",
        "dark horse comics",
...
private val HOME_BLOCKED_TITLE_TERMS =
    listOf(
        "adult",
        "after dark",
        "bad girl",
...
class ComicVineApi {
    ...
    suspend fun searchVolumes(
        ...
        val response =
            client.get("$COMICVINE_BASE_URL/search/") {
                comicVineParameters()
     ...
     */
    suspend fun getPopularSeries(limit: Int = 12): List<ComicVineVolume> = getHomeSafeSeries(limit, HOME_SERIES_MAX_PAGES)
    ...
    suspend fun getRecentIssues(limit: Int = 30): List<ComicVineIssueSummary> =
        getHomeSafeSeries(HOME_ISSUE_VOLUME_COUNT, HOME_SERIES_MAX_PAGES)
    ...
    suspend fun getVolume(id: Int): ComicVineVolume {
        val response =
            client.get("$COMICVINE_BASE_URL/volume/4050-$id/") {
                comicVineParameters()
    ...
    suspend fun getIssuesByVolume(
        volumeId: Int,
        limit: Int = 100,
    ): List<ComicVineIssueSummary> {
        val response =
            client.get("$COMICVINE_BASE_URL/issues/") {
                comicVineParameters()
    ...
    suspend fun getIssue(id: Int): ComicVineIssue {
        val response =
            client.get("$COMICVINE_BASE_URL/issue/4000-$id/") {
                comicVineParameters()
    ...
    private suspend fun getHomeSafeSeries(
        limit: Int,
        maxPages: Int,
    ): List<ComicVineVolume> {
        ...
        repeat(maxPages) {
            val response =
                client.get("$COMICVINE_BASE_URL/volumes/") {
                    comicVineParameters()
                    parameter("field_list", VOLUME_LIST_FIELDS)
    ...
    private suspend fun getRecentIssuesByVolume(
        volumeId: Int,
        limit: Int,
    ): List<ComicVineIssueSummary> {
        val response =
...
class MetronApi {
    ...
    suspend fun searchByUpc(upc: String): MetronPagedResponse<MetronIssueSummary> {
        ...
        val response =
            client.get("$METRON_BASE_URL/issue/") {
                parameter("upc", upc)
            }
